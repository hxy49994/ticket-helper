...
import com.tickethelper.service.GrabForegroundService;
...
public class MainActivity extends AppCompatActivity implements GrabState.StateCallback {
    ...
    private ViewGroup stepIndicators;
    ...
    private void updateServiceStatus(boolean enabled, boolean grabbing) {
        runOnUiThread(() -> {
            if (enabled) {
                ...
                if (grabbing) {
                    btnStartStop.setText("停止抢票");
                    btnStartStop.setBackgroundColor(getColor(R.color.error));
                } else {
                    btnStartStop.setText("开始抢票");
                    btnStartStop.setBackgroundColor(getColor(R.color.damai_red));
                }
    ...
    private void toggleGrabbing() {
        ...
        if (service.isGrabbing()) {
            ...
            addLog("启动抢票流程，配置: 场次" + (config.sessionIndex + 1)
    ...
    public void onStateChanged(int step, String message) {
        runOnUiThread(() -> {
            ...
            if (step == GrabState.STEP_DONE) {
                btnStartStop.setText("抢票完成 ✓");
                btnStartStop.setEnabled(false);
                Toast.makeText(this, "抢票流程完成，已跳转支付宝！", Toast.LENGTH_LONG).show();
            }
        });
    }

    private void updateStepIndicators(int currentStep) {
        // 四个步骤指示器更新（这里简化，直接更新文本）
        TextView[] stepViews = new TextView[4];
        // 由于用线性布局，直接按tag查找（简化处理）
    }
